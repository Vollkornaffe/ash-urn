...
use ash_urn::Base;
use ash_urn::base::queue_families::{DEDICATED_TRANSFER, COMBINED};
use ash_urn::{Command, CommandSettings};

pub fn setup(
    base: &Base,
    n_buffer: u32,
) -> Result<(Command, Command), AppError> {

    let transfer_queue_family_idx = base.queue_map
        .get(&DEDICATED_TRANSFER)
        .unwrap()
        .idx;
    let combined_queue_family_idx = base.queue_map
        .get(&COMBINED)
        .unwrap()
        .idx;
